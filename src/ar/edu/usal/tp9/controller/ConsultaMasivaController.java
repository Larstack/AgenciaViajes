...
//										String.valueOf(paqueteIterado.getImporte())
				...
				if(model.getValueAt(i, 9) != null && !(((String) model.getValueAt(i, 9)).trim().isEmpty())){
					
					HotelesDao hotelesDao = HotelesDao.getInstance();

					Hoteles hotel = hotelesDao.getHotelByNombre((String)model.getValueAt(i, 9));
					...
					if(hotel == null){
						...
						((PaquetesConEstadias) paquete).setEsPensionCompleta(
								Boolean.valueOf(((((String)model.getValueAt(i, 10)).trim().isEmpty()) ||
								((String)model.getValueAt(i, 10)).trim().equals("NO")) ? false : true) 
								);
					}
				}
				...
				if(errores.isEmpty()){
					...
					if(model.getValueAt(i, 1) != null && !(((String) model.getValueAt(i, 1)).trim().isEmpty())){
						...
						for (int j = 0; j < pasajerosArray.length; j++) {
							...
							Pasajeros pasajero = pasajerosDao.getPasajeroByNombre(pasajerosArray[j].trim());
